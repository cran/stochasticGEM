...
static int initial (double *xinit, int ninit, double xl, double xr, int npoint,
	     FUNBAG *lpdf, ENVELOPE *env, double *convex, int *neval,
             METROPOLIS *metrop);

static void sample(ENVELOPE *env, POINT *p);

static void invert(double prob, ENVELOPE *env, POINT *p);

static int test(ENVELOPE *env, POINT *p, FUNBAG *lpdf, METROPOLIS *metrop);

static int update(ENVELOPE *env, POINT *p, FUNBAG *lpdf, METROPOLIS *metrop);

static void cumulate(ENVELOPE *env);

static int meet (POINT *q, ENVELOPE *env, METROPOLIS *metrop);

static double area(POINT *q);

static double expshift(double y, double y0);

static double logshift(double y, double y0);

static double perfunc(FUNBAG *lpdf, ENVELOPE *env, double x);

void display(FILE *f, ENVELOPE *env);

static double u_random();
