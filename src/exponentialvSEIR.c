...
*/
static void expLikelihood_vSEIR(double *parameters, double *infectionTimes,
	...
	double *allTimes, int *indicator, int *SS, int *LL, int *II)
{
	...
	double sumLogBeta=0, sumLogInfections=0, sumInfection=0, sumLatent=0, sumBetaSI=0;
...
Susceptible-Latent-Infectious-Removed MCMC analysis:
	...
	. Exponentially distributed infectiousness periods
*/
SEXP expMH_vSEIR(SEXP N, SEXP removalTimes, SEXP otherParameters, SEXP priorValues,
	...
	SEXP infRateSEIR, remRateSEIR, logLikelihood;
	SEXP parameters;
	...
	double infRate, remRate, latRate, oldLkhood, newLkhood, minimumLikelyInfectionTime,fixedLatent;
	double infRatePrior[2], remRatePrior[2], thetaprior[2];/* priors values */
	double sumSI, sumInfectious, likelihood,logR;
	...
	expLikelihood_vSEIR(REAL(parameters),REAL(infectionTimes),REAL(latencyTimes),
	...
	for(ii = 1; ii <= INTEGER(bayesReps)[0]; ++ii){
		...
		*/
		...
		expLikelihood_vSEIR(REAL(parameters),REAL(infectionCandidate),REAL(latencyCandidate),
			REAL(removalTimes), &INTEGER(N)[0], &nInfected, &nRemoved,
			&sumSI, &sumInfectious, &likelihood,REAL(allTimes),INTEGER(indicator),
			INTEGER(SS),INTEGER(LL),INTEGER(II));
	...
	SET_STRING_ELT(parNames, 2, mkChar("remRateSEIR"));
	SET_STRING_ELT(parNames, 3, mkChar("infectionTimes"));
	SET_STRING_ELT(parNames, 4, mkChar("latencyTimes"));
	...
	SET_ELEMENT(retParameters, 2, remRateSEIR);
	SET_ELEMENT(retParameters, 3, infTimes);
	SET_ELEMENT(retParameters, 4, latTimes);
