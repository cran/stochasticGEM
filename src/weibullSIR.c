...
*/
void weibullLikelihood_SIR(double *parameters, double *infectionTimes,
	...
	double *allTimes, int *indicator, int *SS, int *II)
{
	...
	*sumDurationInfectious = 0;
	...
	for(i = 0; i < *nRemoved; ++i){
		sumDurationDensity+=dweibull((removalTimes[i]-infectionTimes[i]),
			parameters[2], R_pow(parameters[1],-1/parameters[2]), TRUE);
		*sumDurationInfectious+=R_pow((removalTimes[i]-infectionTimes[i]),parameters[2]);
	}
...
Susceptible-Infectious-Removed MCMC analysis:
	. Infectiousness periods are distributed as a Weibull
*/
SEXP weibullMH_SIR(SEXP N, SEXP removalTimes, SEXP otherParameters, SEXP priorValues, SEXP initialValues,
	...
	double sumSI, sumDurationInfectious, likelihood;
	AuxStruct ext;
	...
	for(ii = 1; ii <= INTEGER(bayesReps)[0]; ++ii){
		...
		ext->scale = infScale;
		slice = weiShapelkhoodARS(infShape,ext) - exp_rand();
		lowerLimit=1;upperLimit=100;
		steppingOut(slice,infShape,ext,weiShapelkhoodARS,&lowerLimit,&upperLimit);
		steppingOutShrinkage(slice,&infShape,ext,weiShapelkhoodARS,lowerLimit,upperLimit);
		...
		if(missingInfectionTimes){
			...
			if(log(runif(0,1)) < (newLkhood-oldLkhood)){
...
Susceptible-Infectious-Removed MCMC analysis:
	. Infectiousness periods are distributed as a Weibull
*/
void weibullLkhood_SIR(double *parameters, double *infectionTimes, double *removalTimes,
