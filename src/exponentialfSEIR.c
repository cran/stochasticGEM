...
*/
static void expLikelihood_fSEIR(double *parameters, double *infectionTimes,
...
Susceptible-Latent-Infectious-Removed MCMC analysis:
	...
	. Exponentially distributed infectiousness periods
*/
SEXP expMH_fSEIR(SEXP N, SEXP removalTimes, SEXP otherParameters, SEXP priorValues,
	...
	double infRate, latRate, remRate, oldLkhood, newLkhood, minimumLikelyInfectionTime,fixedLatent;
	...
	expLikelihood_fSEIR(REAL(parameters),REAL(infectionTimes),REAL(latencyTimes),
	...
	for(ii = 1; ii <= INTEGER(bayesReps)[0]; ++ii){
		...
		remRate = rgamma(nRemoved+remRatePrior[0],1/(sumInfectious+remRatePrior[1]));/* update remRate */
		REAL(parameters)[0] = infRate;
		REAL(parameters)[1] = latRate;
		REAL(parameters)[2] = remRate;
		expLikelihood_fSEIR(REAL(parameters),REAL(infectionTimes),REAL(latencyTimes),
		...
		expLikelihood_fSEIR(REAL(parameters),REAL(infectionCandidate),REAL(latencyCandidate),
			REAL(removalTimes), &INTEGER(N)[0], &nInfected, &nRemoved,
			&sumSI, &sumInfectious, &likelihood,REAL(allTimes),INTEGER(indicator),
			INTEGER(SS),INTEGER(LL),INTEGER(II));
		...
		if(updateIniInfGibbs){
			REAL(latencyTimes)[0] = REAL(infectionTimes)[1]-
				rexp(1/(1/fixedLatent+remRate+thetaprior[0]));
